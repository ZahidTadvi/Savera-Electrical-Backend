export class BillingService {
  ...
  async calculateBillAmounts(billData) {
    let subtotal = 0;
    let cgstTotal = 0;
    let sgstTotal = 0;
    let igstTotal = 0;

    const isInterState = (billData.customer?.state || billData.state) !== billData.company?.state;
  ...
  async generateBillNumber(billType, financialYear) {
  ...
  async getSalesReturns(filters, pagination) {
  ...
  async updateSalesReturnStatus(id, status) {
    const index = this.salesReturns.findIndex(ret => ret.id === id);
    if (index === -1) return null;
    
    this.salesReturns[index].status = status;
